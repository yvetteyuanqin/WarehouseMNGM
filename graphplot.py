...
class App(QWidget):
    ...
    def Routing(self):#
        ...
        if self.oneorder==[] or self.x_init==None or self.y_init==None or self.x_end==None or self.y_init==None:
            ...
            self.x_end = 0
            self.y_end = 20
            self.oneorder = [181202,328595,276157,296188,8140,208299]#[1,45,74]
        x_temp=self.x_init
        y_temp=self.y_init
        oneorder=[]
...
if __name__ == '__main__':
    ...
    # ex = App()
    # sys.exit(app.exec_())
    choice = int(input("Hello User, whether you want to visualize an sample order: 1 for yes? 2 for no?"))
